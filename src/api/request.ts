...
const useGetOne = <T>(key: string, url: string, params?: any, formatResult?: any) => {
  const axios = useAxios();

  const service = async () => {
    const data: Result<T> = await axios.get(`${url}`, params);
...
const useCreate = <T, U>(url: string) => {
  ...
  return useMutation(async (params: T) => {
    const data: Result<U> = await axios.post(`${url}`, params);
    return data;
  });
};
